...
function BentoCard({
  ...
  return (
    <motion.div
      ...
      layoutId={layoutId}
      className={cn(
        "relative border-4 border-black p-6 shadow-lg cursor-pointer overflow-hidden h-full flex flex-col",
        className
      )}
    >
      <div className="relative">
        {icon ? (
          <div className="mb-6 bg-white p-3 inline-block border-2 border-black">
            {icon}
          </div>
        ) : null}
        <h3 className="text-2xl font-bold mb-2">{title}</h3>
        {subtitle ? (
          <p className="font-medium text-sm text-muted-foreground mt-3 border-l-5 pl-2">
            {subtitle}
          </p>
        ) : null}
      </div>
